...
- **Profile-scoped storage (#530/#531)** — profiles are a *filesystem* dimension, never a schema dimension: each profile owns its own `cache.db` (existing schema, zero profile columns/migrations) plus settings files under `profiles/<id>/`, encrypted with its own SQLCipher key (keychain account `master-key:<profile-id>`; the MCP bearer token is likewise keyed `bearer-token:<profile-id>`). **All path construction goes through `unkai_store::paths::ProfilePaths`** — never build `dirs::config_dir()/unkai-mail/...` paths inline; loaders (`app_settings`, `settings_sync`) take the file path as an argument.
- **Machine-scoped storage (#532)** — the one deliberate exception to the profile dimension: `<config_dir>/unkai-mail/shared.db` (`unkai_store::SharedCache`, plaintext SQLite, own append-only migration ladder in `shared_cache.rs`) holds data that is identical for every profile and carries no user content — today exactly the URLhaus feed snapshot (`urlhaus_urls`/`urlhaus_meta`, one hourly download per machine via ONE process-wide `urlhaus_refresh_worker` spawned in `.setup()`, never per profile). The scope rule is binary and documented on both structs: `Cache` = profile-scoped (everything account-keyed plus personal account-less tables — `geocode_cache` stays per-profile *on purpose*, queried locations are personal), `SharedCache` = machine-scoped public data. Classify any new account-less table against that rule explicitly. Every `AppContext` carries a clone of the same `SharedCache` (`ctx.shared`) — threaded explicitly, no globals — and deleting a profile must never touch `shared.db`.
- **Multi-profile runtime (#533)** — one process hosts N profile contexts; every command, loop, and event is profile-scoped, and there are **no process globals** for profile state (`GLOBAL_CACHE` / `active_profile()` are gone — don't reintroduce them). `src-tauri/src/registry.rs` owns the `ProfileRegistry` (window label → profile, profile → `ProfileHandle` bundling the `AppContext`, per-profile `McpServer`, sync plumbing, and background-loop handles; `build_profile_handle` constructs the whole set). **Every `#[tauri::command]` shim that touches profile state takes `window: tauri::Window, reg: State<'_, ProfileRegistry>` and resolves once via `profile_ctx(&window, &reg)?`** — never inline map lookups; pass the body exactly what it needs from the handle (`&h.ctx.cache`, `&h.ctx`, `&h.ctx.profile`, …). Window-label→profile resolution lives only in `src-tauri`; `unkai-commands` never sees a window label. The tray is the one shared surface: one icon, aggregate unread badge across profiles.
- **Profile windows (#535)** — one *primary* window per profile: the static `"main"` window hosts the startup profile, additional profiles live in Rust-created `profile-*` windows (`src-tauri/src/windows.rs` is the only place that creates/finds/raises them — outside `.setup()`'s boot-time handling of the static main window, never `get_webview_window("main")` directly; raise-the-app surfaces go through `windows::show_primary_window`, which targets the most recently focused primary window). Switch-in-place = registry remap of the window's label (labels are immutable) + `App.svelte`'s `switchProfile` state reset; the backend also closes the outgoing profile's popouts so their parent-targeted handoffs can't leak into the new profile's shell. Close-to-tray hides ONLY the static main window; `profile-*` windows destroy on close — a hidden window would keep its label mapped and make its profile undeletable. Closing a profile's last window does NOT shut its context down (background sync + the aggregate tray badge continue; native toasts pause until a window reopens — known limitation); only `delete_profile` (which tears down window-less contexts via `registry::shutdown_profile_context`) and app exit do. Every registry read-modify-write goes through `unkai_store::profiles::update_registry` (serialised — concurrent writers would lose updates). Frontend event listeners are window-targeted (`api/events.ts` listens on the current webview window), backend pushes use `emit_to` per profile label, and popout→main handoffs use `emitAppEventToParent` (broadcast fallback when the parent window is gone) — a bare broadcast fires in every profile's shell, so treat `emitAppEvent` as a last resort. Profile-scoped `localStorage` keys are namespaced `unkai.<profile-id>.<suffix>` via `profileLocalStorage.ts` (all windows share one origin); machine-global keys stay bare.
- **Profile window chrome & external-open routing (#536)** — three "exactly one module" rules from the chunk-5 cleanup, keep them that way:
  - **`src-tauri/src/external_open.rs`** is the single place OS-handed payloads (`mailto:` URLs, `.eml`/`.ics` paths) are captured, buffered, and routed. The routing rule lives in `route_external_open`: most recently focused primary window, else a (re)created startup-profile window. Never add argv scanning, deep-link handling, or pending-payload state to `main.rs` — the cold-start drains (`take_pending_mailto_urls` / `take_pending_files_to_open`) and the targeted `unkai://mailto` / `unkai://open-file` events (frontend twin: `processPendingMailtoUrls` / `processPendingFileOpens` in `App.svelte`, TTL-deduped because the backend always buffers AND emits) are the only delivery paths.
  - **Profile icons render in exactly two places**: `ProfileGlyph.svelte` for DOM surfaces (rail bubble, switcher popover, transition screen — never branch on `icon.kind` in a component) and `profileIcon.ts` for canvas/title work (window-icon compositing + `profileWindowTitle`). The composited PNG goes to the backend via `set_window_identity` (body in `windows.rs`); the frontend rasterizes because the webview can draw colour emoji and Rust can't. On Windows, once >1 profile exists EVERY primary window gets a per-profile AUMID (`com.unkai.mail.profile.<id>`) — the startup profile too, because a grouped taskbar button's icon comes from the AUMID's shortcut/icon-cache, not the live window icon, so a base-AUMID startup window shows the plain logo while its siblings show badges (tried, reverted). A pinned shortcut is then a launcher, not a merged button. Single-profile installs keep base `com.unkai.mail` + classic pin behaviour, and stay unbadged and untitled ("Unkai Mail", plain logo).
  - **`FullscreenState.svelte` is the ONE full-viewport wait screen**: the boot/loading variant (app logo + status line) and the profile-switch transition (from/to glyphs + flow dots) are the same component — no bare "Loading…" divs, ever. `App.svelte`'s `profileTransition` state raises it for the whole switch-in-place sequence and a dismissal `$effect` drops it on the new shell's first frame — never on an animation timer, and immediately when the target profile is locked (the LockScreen must become interactive).
...
- **Commands**: one typed wrapper per `#[tauri::command]`, grouped into domain modules (`api/mail`, `api/compose`, `api/accounts`, `api/contacts`, `api/calendar`, `api/nextcloud`, `api/talk`, `api/notes`, `api/profiles`, `api/tasks`, `api/crypto`, `api/settings`, `api/system`), all funnelling through `call()` in `api/core.ts`. Components do `import * as api from './api'` and call `api.mail.fetchEnvelopes({ accountId, folder, limit })`. **When you add/rename/change a Rust command, update its wrapper in the matching domain module in the same PR** — that's the point of the layer: the compiler finds every affected call site.
...
- **Allowed exceptions** (window plumbing only, enforced by the guard test): `standalone*Window.ts` files may use `@tauri-apps/api/webviewWindow` — in practice only the shared popout helper `standalonePopoutWindow.ts` (#535) still does; the per-kind wrappers and `attachmentOpen.ts` go through it. Standalone components may use `@tauri-apps/api/window` to close themselves; type-only imports from `@tauri-apps/api/event` are fine anywhere. **Never spawn a popout by hand** — `openPopout()` owns the payload stash, the label, the parent-window threading, and the profile registration; a hand-rolled `new WebviewWindow` silently binds the popout to the startup profile.
...
**The authoritative design-language spec is [`docs/UI_CONVENTIONS.md`](docs/UI_CONVENTIONS.md) — read it before writing or editing any Svelte component.** It carries the hard rules (token-only color, the glass-utility mandate, radius tiers, the three button shapes, icon/emoji/glyph bans, motion and density limits), a grep-able ban list, and the drift ledger of known violations to fix lazily. The sections below remain authoritative for behavioral idioms (menus, anchoring, dismissal, view-shell structure). Where the two disagree on a visual question, `docs/UI_CONVENTIONS.md` wins — notably: **destructive color is now `error-*` tokens, not raw `red-*`** (lazy migration; never add new `red-*`).
...
- **`.popover-opaque`** — floating layers that render *inside* a glass surface (popovers/menus anchored in the Compose / EventEditor modals or the editor canvas), where stacking a second `backdrop-filter` is banned. Opaque surface fill + 1px border + the same `--glass-shadow`. These two utilities are the only legal float recipes — see `docs/UI_CONVENTIONS.md` rule 2.
...
- **Small action buttons are icon-only with `preset-outlined-surface-500` as the base, all the same shape.** For per-row / per-setting actions (save, edit, replace, remove, cancel, configure) use the project's standing icon-button vocabulary rather than tonal/filled buttons with text labels. The base shape is identical across neutral and destructive — only the hover overlay differs — so a Save next to a Cancel next to a Remove read as siblings, not as different button kinds.
  ...
  - **Destructive variant** (remove, delete, drop): same base + `hover:bg-error-500/15 hover:text-error-500 hover:border-error-500/40`, paired with the `trash` icon. (Older code uses raw `red-500` here — that's drift; swap to `error-*` when touching the file.) Stays neutral at rest, turns red only on hover — so the destructive action doesn't visually shout from the page. Canonical refs: the Trusted Senders remove row in `AccountSettings.svelte`, the Remove key button in `EncryptionSettings.svelte`.
