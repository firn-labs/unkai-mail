...
- **Profile window chrome & external-open routing (#536)** — three "exactly one module" rules from the chunk-5 cleanup, keep them that way:
  ...
  - **Profile icons render in exactly two places**: `ProfileGlyph.svelte` for DOM surfaces (rail bubble, switcher popover, transition screen — never branch on `icon.kind` in a component) and `profileIcon.ts` for canvas/title work (window-icon compositing + `profileWindowTitle`). The composited PNG goes to the backend via `set_window_identity` (body in `windows.rs`); the frontend rasterizes because the webview can draw colour emoji and Rust can't. On Windows, `windows::set_window_app_user_model_id` gives non-startup-profile windows a per-profile AUMID (`com.unkai.mail.profile.<id>`) for their own taskbar group; the startup profile keeps `com.unkai.mail` so the pinned/Start-Menu shortcut keeps highlighting it. Single-profile installs stay unbadged and untitled ("Unkai Mail", plain logo).
