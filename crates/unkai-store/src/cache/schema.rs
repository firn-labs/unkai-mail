...
const MIGRATIONS: &[&str] = &[
    ...
    //   recurrence expander in `unkai_caldav::expand`) can
    ...
    // `<config-dir>/unkai-mail/nextcloud_accounts.json` into the
    ...
    // verifier in `unkai-core::tls` covers Nextcloud HTTPS too
    ...
    //   * `replied_kind` — Unkai-only metadata recording *how*
    ...
    //     `NULL` means "we didn't reply via Unkai" — fall back
