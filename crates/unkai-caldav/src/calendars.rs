...
pub async fn create_calendar(
    ...
    let method = Method::from_bytes(b"MKCALENDAR")
        .map_err(|e| UnkaiError::Other(format!("MKCALENDAR method: {e}")))?;
    let resp = http
        .request(method, &url)
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("MKCALENDAR {url}: {e}")))?;
...
pub async fn update_calendar(
    ...
    let method = Method::from_bytes(b"PROPPATCH")
        .map_err(|e| UnkaiError::Other(format!("PROPPATCH method: {e}")))?;
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("PROPPATCH {calendar_url}: {e}")))?;
