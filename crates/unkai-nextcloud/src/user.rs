...
pub async fn fetch_current_user(
...
) -> Result<NextcloudUserProfile, UnkaiError> {
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("user request failed: {e}")))?;
    ...
    let env: OcsEnvelope<UserData> = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("user info bad JSON: {e}")))?;
...
pub async fn find_user_by_email(
    ...
    email: &str,
    trusted_certs: &[TrustedCert],
) -> Result<Option<NextcloudUserMatch>, UnkaiError> {
    let server = client::normalize_server_url(server_url);
    let url = format!(
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("sharees request failed: {e}")))?;
...
pub async fn fetch_my_groups(
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("user-groups request failed: {e}")))?;
    let status = resp.status();
    if !status.is_success() {
        return Err(UnkaiError::Nextcloud(format!(
    ...
    let env: OcsEnvelope<GroupsListData> = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("user-groups bad JSON: {e}")))?;
...
pub async fn fetch_group_member_ids(
    ...
    let env: OcsEnvelope<GroupMembersData> = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("group-members bad JSON: {e}")))?;
...
pub async fn fetch_user_profile(
    ...
    target_user_id: &str,
    trusted_certs: &[TrustedCert],
) -> Result<NextcloudUserSummary, UnkaiError> {
    let server = client::normalize_server_url(server_url);
    let url = format!(
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("user-profile request failed: {e}")))?;
    let status = resp.status();
    if !status.is_success() {
    ...
    let env: OcsEnvelope<UserProfileData> = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("user-profile bad JSON: {e}")))?;
...
pub async fn fetch_my_circles(
    ...
    app_password: &str,
    trusted_certs: &[TrustedCert],
) -> Result<Vec<NextcloudCircle>, UnkaiError> {
    let server = client::normalize_server_url(server_url);
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("circles request failed: {e}")))?;
    let status = resp.status();
    ...
    let env: CirclesEnvelope = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("circles bad JSON: {e}")))?;
    Ok(env
        .ocs
        .data
        .into_iter()
...
pub async fn fetch_circle_member_ids(
    ...
    let resp = http
        ...
        .send()
        .await
        .map_err(|e| UnkaiError::Network(format!("circle members request failed: {e}")))?;
    let status = resp.status();
    if status == reqwest::StatusCode::NOT_FOUND || status == reqwest::StatusCode::FORBIDDEN {
        return Ok(Vec::new());
    }
    ...
    let env: CircleMembersEnvelope = resp
        .json()
        .await
        .map_err(|e| UnkaiError::Protocol(format!("circle members bad JSON: {e}")))?;
    Ok(env
        .ocs
        .data
        .into_iter()
